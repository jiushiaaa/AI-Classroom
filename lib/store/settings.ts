  ...
  persist(
    ...
    },
    {
      ...
      migrate: (persistedState: unknown, version: number) => {
        ...
        const agentModeRaw = (state as Record<string, unknown>).agentMode;
        if (agentModeRaw === 'preset') {
          (state as Record<string, unknown>).agentMode = 'custom';
        }
        if (
          (state as Record<string, unknown>).agentMode === undefined ||
          (state as Record<string, unknown>).agentMode === null
        ) {
