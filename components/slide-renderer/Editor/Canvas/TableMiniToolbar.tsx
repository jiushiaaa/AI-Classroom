...
import { ElementTypes, type PPTElement, type PPTTableElement } from '@/lib/types/slides';
...
export function TableMiniToolbar() {
  ...
  const target = useMemo<PPTTableElement | undefined>(() => {
    if (activeElementIdList.length !== 1) return undefined;
    const el = elements.find((e) => e.id === activeElementIdList[0]);
    if (!el || el.type !== ElementTypes.TABLE) return undefined;
    return el as PPTTableElement;
  ...
  const recompute = useCallback(() => {
    ...
    if (!wrapper) return;
    // Table is wrapped like the image: hidden siblings + ContextMenu, so the
    // sized .editable-element-table is never a direct child of #editable-element-*.
    const sized =
      (wrapper.querySelector('.editable-element-table') as HTMLElement | null) ?? wrapper;
  ...
  const node = (
    <div
      ref={barRef}
      data-testid="table-mini-toolbar"
      role="toolbar"
      aria-label={t('editMode.miniToolbar.tableToolbarAriaLabel')}
      style={{
        position: 'fixed',
        top: coords.top,
        left: coords.left,
