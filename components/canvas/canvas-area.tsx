...
export function CanvasArea({
  ...
  // In edit mode the slide canvas is fully interactive (PPTist Canvas); we
  // don't want clicks on the canvas to also trigger play/pause, and we don't
  // want the play-hint overlay covering editable elements.
  const showControls = mode === 'playback' && !whiteboardOpen && !isEditing;
  // Play-hint overlay: only show when the user has explicitly paused. The
  // default 'idle' state (before any interaction) keeps the slide clean —
  // clicking the slide still triggers playback via handleSlideClick.
  const showPlayHint =
    showControls &&
    engineState === 'paused' &&
        ...
        >
          ...
          {currentScene && !whiteboardOpen && (
            <div className="absolute inset-0">
              <SceneProvider>
                <SceneRenderer scene={currentScene} mode={mode} editing={isEditing} />
              </SceneProvider>
            </div>
          )}
