...
import { QuizEditor } from '@/components/scene-renderers/quiz-editor';
...
// P3: `editing` flag is propagated from <SceneRenderer editing> so the
// publisher can WYSIWYG-edit question text & options. Optional with default
// false — behaves like the legacy playback-only quiz view when unset.
...
function QuestionCard({
  ...
  return (
    <motion.div
      // Inline-AI markers: the controller listens for clicks on
      // [data-quiz-question-id] to anchor the cmd-K popup to a specific
      // question, and quotes the question text in the AI prompt prefix.
      // The DOM `id` lets the reposition effect re-resolve the rect on
      // scroll without per-kind prefix knowledge.
      id={`quiz-question-${question.id}`}
      data-quiz-question-id={question.id}
      data-quiz-question-text={question.question}
...
export function QuizView({ questions, sceneId, editing = false }: QuizViewProps) {
  // P3: when the publisher is in edit mode, hand off to the dedicated
  // editor so they can mutate question text / options / correct answers.
  if (editing) {
    return <QuizEditor sceneId={sceneId} questions={questions} />;
  }
  return (
    <QuizViewBody questions={questions} sceneId={sceneId} />
  );
}

function QuizViewBody({ questions, sceneId }: Omit<QuizViewProps, 'editing'>) {
